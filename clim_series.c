...
void set_cru_clim(
	...
	if(grid->climy<=cru_te){
		...
		for(h=0;h<ASTEP;h++){
			/* temperature */
			tmp_var = grid->ncep_tmp2m[grid->climy - 1948][h][grid->ncep_lat][grid->ncep_lon] 
			...
			/* precipitation */
			pre_var = grid->ncep_prate[grid->climy - 1948][h][grid->ncep_lat][grid->ncep_lon] 
			...
			/* cloudiness */
			tcdc_var = grid->ncep_tcdc[grid->climy - 1948][h][grid->ncep_lat][grid->ncep_lon] 
