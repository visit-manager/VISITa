...
void publish_cbud(
	...
	for(f=0;f<ASTEP;f++){
		...
		fprintf(result,"%.4lf ", flux->nbp[f]);	/* added (2010/02/01 ) */
	...
	fprintf(result,"%.3lf ", flux->npp_schuur); /* added 2008/09/08  */
	fprintf(result,"%.3lf ", flux->npp_nceas);  /* added 2008/09/08  */

	fprintf(result,"%.3lf ", flux->npp_chikugo);  /* added 2014/02/14  */
	fprintf(result,"%.3lf ", flux->npp_madison_parwsi);  /* added 2014/02/14  */
	fprintf(result,"%.3lf ", flux->npp_madison_gddswsi);  /* added 2014/02/14  */
	fprintf(result,"%.3lf ", flux->npp_madison_tp);  /* added 2014/02/14  */
	
	fprintf(result,"%.3lf ", flux->erod_soil);
	fprintf(result,"%.3lf ", flux->erod_orgmat);
	fprintf(result,"%.3lf ", flux->erod_carbon);
...
void f_output_result(
    ...
    if(OUTPUT_CARBON1 == 1){
        ...
        for(f=0;f<ASTEP;f++){
            ...
            fprintf(fp_o[0],"%.4lf ",  flux->nbp[f]); /* */ /* added  (2010/01/20) */
    ...
    if(OUTPUT_GHG == 1){
        ...
        for(f=0;f<ASTEP;f++){
            fprintf(fp_o[3],"%.4lf ", loct->msww[f]);
            fprintf(fp_o[3],"%.4lf ", loct->wfps[f]);
    ...
    if(OUTPUT_BB == 1){
        ...
        for(f=0;f<ASTEP;f++){
            fprintf(fp_o[4],"%.4lf ", loct->msw30[f]);
            fprintf(fp_o[4],"%.4lf ", flux->day_fire[f]);
            fprintf(fp_o[4],"%.4lf ", flux->a_burnt[f]);
            ...
            fprintf(fp_o[4],"%.4lf ", flux->wa_burnt[f]); /* wood burnt: 2017/11/30 */
            fprintf(fp_o[4],"%.4lf ", flux->bb_n2o_litter[f]+flux->bb_n2o_leaf[f]+flux->bb_n2o_wood[f]+flux->bb_n2o_root[f]); /*  */
        }
    ...
    if(OUTPUT_HYDMET == 1){
        ...
        for(f=0;f<ASTEP;f++){
            ...
            fprintf(fp_o[6],"%.3lf ", grid->prate_sfc[f]);
            fprintf(fp_o[6],"%.3lf ", grid->gl_rad[f]);
            fprintf(fp_o[6],"%.3lf ", grid->par[f]);
            fprintf(fp_o[6],"%.3lf ", grid->par_bp[f]);
            fprintf(fp_o[6],"%.3lf ", grid->par_dp[f]);
            fprintf(fp_o[6],"%.3lf ", loct->vpd[f]);
            fprintf(fp_o[6],"%.3lf ", loct->rad_net_long[f]);
            fprintf(fp_o[6],"%.3lf ", loct->rad_net_short[f]);
            fprintf(fp_o[6],"%.3lf ", loct->msw30[f]);
            fprintf(fp_o[6],"%.3lf ", loct->msww[f]);
            fprintf(fp_o[6],"%.3lf ", loct->incep[f]);
            fprintf(fp_o[6],"%.3lf ", loct->evpr[f]);
            fprintf(fp_o[6],"%.3lf ", loct->trspr[f]);
            fprintf(fp_o[6],"%.3lf ", loct->ro2[f]); /* */
            
            fprintf(fp_o[6],"%.4lf ", loct->fappfd_g[f]); /* */
            
            /* added 2012/12/19  */
            fprintf(fp_o[6],"%.4lf ", (echar->soil).albedo[f]); /* */
            fprintf(fp_o[6],"%.4lf ", loct->albedo_sfc[f]); 
            fprintf(fp_o[6],"%.3lf ", loct->msnwa[f]);
            
            /* added 2013/12/16  */
            fprintf(fp_o[6],"%.3lf ", grid->par_be[f]);
            fprintf(fp_o[6],"%.3lf ", grid->par_de[f]);
            fprintf(fp_o[6],"%.4lf ", loct->appfd_g[f]);
	...
	/* carbon isotopes: added  (2009/07/14) ************/
    if(OUTPUT_ISOTOPE == 1){
	...
	/* carbon supplementary: added  (2011/08/18) ***********/
    if(OUTPUT_CARBON2 == 1){
        ...
        for(f=0;f<ASTEP;f++){
            ...
            if(ISIMIP_RUN == 2){
                ...
                fprintf(fp_o[8],"%lf ", (echar->soil).rl/1000.0); //17
                fprintf(fp_o[8],"%lf ", (echar->soil).ft_l[f]);
                fprintf(fp_o[8],"%lf ", (echar->soil).fm_l[f]);
                
                fprintf(fp_o[8],"%lf ", (echar->soil).rh/1000.0); //20
                fprintf(fp_o[8],"%lf ", (echar->soil).ft_h[f]);
                fprintf(fp_o[8],"%lf ", (echar->soil).fm_h[f]);
            }
        }
        fprintf(fp_o[8],"\n");
    }
...
void f_grid_av(
	...
	short period;
	double fweight, fweight_nat, fweight_wet, fweight_pad, length;
    ...
    if(IMPRESSIONS_RUN == 0){
        ...
        if(grid->climy>=1950 && grid->climy<1960){
            ...
            period = 1; length = 30.0;
        }else if(grid->climy>=2020 && grid->climy<2030){
            period = 2; length = 10.0;
        }else if(grid->climy>=2050 && grid->climy<2060){
            period = 3; length = 10.0;
        }else if(grid->climy>=2080 && grid->climy<2090){
            period = 4; length = 10.0;
        }else{
            period = -1; length = 1.0;
        }
    }
	...
	if(period>=0){
		...
		if(DF97==1){
			g_gpp13[period][grid->row][grid->col] = 
					d13c_addition((flux->plant).d13c_gpp[grid->m], (flux->plant).gpp_df97[grid->m] /length,
					g_gpp13[period][grid->row][grid->col], g_gpp[period][grid->row][grid->col]);
		}else{
			g_gpp13[period][grid->row][grid->col] = 
					d13c_addition((flux->plant).d13c_gpp[grid->m], (flux->plant).gpp[grid->m] /length,
					g_gpp13[period][grid->row][grid->col], g_gpp[period][grid->row][grid->col]);
		}
		g_er13[period][grid->row][grid->col] = d13c_addition(flux->d13c_er[grid->m], flux->er[grid->m] /length,
		...
		if((flux->plant).gpp[grid->m]>0.0){
			g_gpp14[period][grid->row][grid->col] = (g_gpp14[period][grid->row][grid->col]
                            ...
                            * (flux->plant).gpp[grid->m] /length) / (g_gpp[period][grid->row][grid->col]
                            + (flux->plant).gpp[grid->m] /length);
		}
		if(flux->er[grid->m]>0.0){
			g_er14[period][grid->row][grid->col] = (g_er14[period][grid->row][grid->col]
                            * g_er[period][grid->row][grid->col] + flux->d14c_er[grid->m] * flux->er[grid->m]
                            /length) / (g_er[period][grid->row][grid->col] + flux->er[grid->m] /length);
		}
#endif
