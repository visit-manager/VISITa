...
/***************************************************/
#define VEG_NUM 34 /* total vegetation number */

/***************************************************/
#define CRU_PD 100 	/* 100: AD 1901 - 2000 */
/* #define GCM_PD 99	*/	/* 99 : 2001-2099 */
#define GCM_PD 100 	/* 99 : 2001-2099 */

#define PIVOT_CO2Y 1901
/* cru-init 1901 */ /* default for 031110 */
/* con 1990 */
/* dcd 2081 */
/* NCEP1 1948 */
#define PIVOT_CLIMY 1901 
/* 1901: CRU */ /* default for 031110 */
/* 1990: control */
/* 1948: control */
...
#define TM 0
/* 0: as it is */
/* 1: uniform warming by 1¡C */
/* 2: gradual rise */
/* 3: descrete warming */
/* 4: spike rise */
/* 5: uniform cooling by 1¡C */
/* 6: descrete cooling */
...
struct Grid{ 
	...
	long 	row;			/* grid order, raw and column in 0.5 degree grid */
	long 	col;			
	long 	gcm_row;			/* grid order, raw and column in GCM's grid */
	long 	gcm_col;			
	...
	double 	d13C_bCO2[12];			/* stable carbon isotope composition of background CO2, dimensionless */
	...
	double 	ugrd_10m_a[12];
	double	vgrd_10m_a[12];			/* wind velocity, m s-1 */
	double 	rad_a[12];			/* solar radiation, W m-2 */
	double 	par_a[12];			/* PAR, micro mol m-2 yr-1 */
	
	double 	prec_sub_a[12];			/* precipitation, mm mon-1 */
	...
	/* UEA/CRU TS2.0 data */
	long	cru_exist;			
	double	hist_tmp[CRU_PD][12];			
	double	hist_pre[CRU_PD][12];			
	double	hist_cld[CRU_PD][12];			
	double	hist_vap[CRU_PD][12];			
...
struct Pchar{ 
	...
	double	malloc_f[12];
	double	malloc_c[12];
	double	malloc_r[12];			/* monthly allocation ratios */
	...
	double	topt;
	double	topt0;				/* optimum temperature, deg C*/
	...
	double	gc[12];			/* monthly stomatal conductance, mmol H2O m-2 s-1 */
	double	gs_b0;
	double	gs_b1;
	...
	double	rgf;
	double	rgc;
	double	rgr;			/* specific growth respiration rate, g C g C-1 alloc */
	double	rmf;
	double	rmc;
	double	rmr;			/* specific maintenance respiration rate at 15 degC, mg C g C-1 day-1 */
	double	rmf0;			/* specific maintenance respiration rate at 15 degC, mg C g C-1 day-1 */
	double	rmc_s;
	double	rmr_s;			/* specific maintenance respiration rate of sapwood at 15 degC, mg C g C-1 day-1 */
	double	rmc_h;
	double	rmr_h;			/* specific maintenance respiration rate of heart wood at 15 degC, mg C g C-1 day-1 */
	double	qTf[12];
	double	qTc[12];
	double	qTr[12];			/* temperature dependence, dimensionless */
	
	/*** litter fall ***/
	/* specific litter fall rate, fraction */
	double	lf[12];
	double	lc[12];
	double	lr[12];			/* specific litter fall rate, fraction */
	...
	double	qTf0;
	double	qTc0;
	double	qTr0;			/* temperature dependence, dimensionless */
	/*** litter fall ***/
	double	lf0;
	double	lc0;
	double	lr0;			
...
struct Sflx{	 
	...
	/* CH4 oxydation ********/
	double	ch4oxy_ridg[12];			
	double	ch4oxy_casa[12];			
	double	ch4oxy_delgrosso[12];			
