...
void f_erosion(
	...
	if(grid->prate_sfc_ann<=850.0){
		...
		grid->f_erosion_r = 587.8 - 1.219*grid->prate_sfc_ann + 0.004105*grid->prate_sfc_ann*grid->prate_sfc_ann;
	}
	...
	if(VEGCOVER==0){
		...
		grid->f_erosion_c = (1.0 - grid->f_crop)*c_factor_v[grid->veg_sage]*(1.6 - fveg_a) + grid->f_crop*0.5;
	}
	...
	grid->f_erosion_p = (1.0-grid->f_crop)*p_factor_v[grid->veg_sage] + grid->f_crop*consv;
